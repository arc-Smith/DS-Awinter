class HashMapNode:
    # INITIALIZER
    def __init__(self, key, val, next=None): # O(1) time
        self.key = key
        self.val = val # O(1) time
        self.next = next # O(1) time
        
    # GETTERS
    def get_key(self): # O(1) time
        return self.key # O(1) time
    ...
    # SETTERS
    def set_key(self, key): # O(1) time
        self.key = key # O(1) time
...
class SinglyLinkedList:
    # INITIALIZER
    def __init__(self, key, val): # O(1) time
        self.head = HashMapNode(key, val) # this ALWAYS points at the first node of the LinkedList | O(1) time
    ...
    def insert_at_back(self, newKey, newVal): # O(n) time
        # Step 1: Create the Node that we want to add
        newNode = HashMapNode(newKey, newVal) # O(1) time
    ...
    def insert_at_middle(self, newKey, newVal):
        # Step 1: Create new node
        newNode = HashMapNode(newKey, newVal)
